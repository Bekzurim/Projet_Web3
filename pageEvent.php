	...
	<div class="col">
        ...
        <div>
			...
			<h2> <?php echo $event["nom_event"]; ?> </h2>
					...
					<?php 
					$query="SELECT * FROM Participer WHERE id_annonceur=".$_SESSION["id"]." AND id_evenement=".$id;
					$result = mysqli_query($con, $query);
					$parti = $result->fetch_array();
					if($parti!=NULL){
            $bouton='<div class="row">
            <form class="col-6" action="participer.php" method="post">
			<div>
            <input type="hidden" name="id1" value="'.$_SESSION["id"].'" />
            <input type="hidden" name="id2" value="'.$event["id_evenement"].'" />
			<input type="hidden" name="id3" value="'.$event["id_type"].'" />
            <input type="hidden" name="id4" value="unparticipe" />
            <input type="submit" value="Ne plus participer" />
          </div>
        </form></div>';
          } else {
            $bouton='<form action="participer.php" method="post">
            <div>
              <input type="hidden" name="id1" value="'.$_SESSION["id"].'" />
            <input type="hidden" name="id2" value="'.$event["id_evenement"].'" />
			<input type="hidden" name="id3" value="'.$event["id_type"].'" />
              <input type="hidden" name="id4" value="participe" />
